...
subsequent number is the previous one doubled, starting from the number 1.
...
The next is 2 times 2, which is 4. The next is 4 times 2, which is 8, and so forth.
Each power in the sequence is multiplied by two to produce the next one.
"""


def main():
    result = powerOfTwo(100)
    for num in result:
        print(num)


def powerOfTwo(upperLimit):
    powersOfTwo = [1]
    for num in range(1, upperLimit):
        if 2 ** num <= upperLimit:
            powersOfTwo.append(2 ** num)
    return powersOfTwo
